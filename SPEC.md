...
| F-411 | Tool Use for Port APIs | Expose port42 bridge APIs (terminal, clipboard, screen, filesystem, browser, automation, etc.) as LLM tool definitions. Companions can call them via tool use during normal conversation without creating a port. Same permission system applies. "Read my clipboard" or "take a screenshot" just works in chat. | Companion calls port42.clipboard.read as a tool during conversation | | |
