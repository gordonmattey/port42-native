...
var phKey="%s";
if(phKey){posthog.init(phKey,{api_host:"https://ph.port42.ai",autocapture:false});posthog.capture("invite_page_viewed");document.querySelectorAll(".panel-btn").forEach(function(b){b.addEventListener("click",function(){posthog.capture("invite_clicked",{action:b.textContent.trim().toLowerCase()})})})}
</script>
</body>
</html>
`
