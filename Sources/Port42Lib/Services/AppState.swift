...
public final class AppState: ObservableObject {
    ...
    func bakeCompanionPrompt(name: String, spaceId: String, systemPrompt: String?) -> String {
        ...
        return Self.companionPromptText(name: name, spaceId: spaceId, spaceName: spaceName,
                                        gatewayPort: gwPort, systemPrompt: systemPrompt)
    }

    /// The prompt text itself, pure so the C3 gate can scan it without an app.
    ///
    /// **This is the first thing every Port42-spawned companion reads**, and until 2026-07-31 it
    /// taught an UNAUTHENTICATED call: a bare curl with no header, at a door 5b had already closed.
    /// A session that needed to call therefore went looking for a credential it could read and found
    /// another tool's. After B every companion has its own token file, so telling it so is honest.
