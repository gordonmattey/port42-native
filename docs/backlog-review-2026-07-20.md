...
| 2.3 | **App froze mid-demo, every port grey** | ? | **CAUSE OPEN — RCA in `docs/rca-app-freeze-2.3.md`.** Solid: empty space healthy; non-empty spaces (testing, fundemos) hang in a deep non-terminating `ConversationContent.body` layout recursion (~429-char stack). The earlier "background port confirmed" conclusion is **RETRACTED** — proven wrong (a fresh launch with the background cleared still hung; `restoreBackgroundPort` reads only `shell.backgroundPortId`). Tests were contaminated by 16 ambient companion CLIs mutating live spaces + 3 background-persistence sources. The scroll-tracking fix attempt was reverted (didn't work). Next: a CONTROLLED experiment (quiet the ambient CLIs; build up from empty / bisect a space's saved panels one at a time). |
...
| 3.4 | **Native terminal output-streaming bridge (`onFlush`)** — SHIPPED (2026-07-23) via the L1 bus | S–M | The `onFlush` no-op is now a publish to the NotifyBus (kind `terminal.output`, non-hooks tools only), threaded through `GhosttyTerminalController.onOutput`. `port.subscribe(id)` streams it — no separate `terminal_stream` verb needed; it rides the unified Subscribe→Notify (`plan-port42-protocol-local-bus.md` L1). |
