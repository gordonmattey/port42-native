...
- **One native chat port per space** (`isChatPort`, rendered by `ChatView` and `ConversationContent`,
...
that port's chat as a panel attached to the port. A space is a port, so the space's own chrome opens
...
`ChatView`, `ConversationContent`, the space's native chat port, inline ports and port fences (D11),
...
3. The router subscribes companions and the shim replies into the chat. Scenario 1 moves to a space's
   chat.
4. Remove the space's native chat port, the native chat views and the `messages` table.
5. Port 0's chat.
...
2. **The panel slides down from the companion bar.** The bar at the top of a port shows who is in its
   chat as profile pictures, the way the native chat's header does today. Clicking it slides the chat
   down over the port.
